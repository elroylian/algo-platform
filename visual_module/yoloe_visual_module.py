...
def create_model_with_visual_prompt(source_image_np, visuals: dict, class_names: list):
    ...
    model.predict(temp_filename, prompts=visuals, predictor=YOLOEVPSegPredictor, device = device)

    # Second stage: get the VPE returned
    model.predict(temp_filename, prompts=visuals, return_vpe=True, device = device)
...
def predict_with_model(model, target_image_np, class_names):
    ...
    """
    results = model.predict(target_image_np, device=device, conf=0.4, iou=0.6)
